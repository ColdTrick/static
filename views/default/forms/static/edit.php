...
if (!empty($parent_options)) {
	...
	$form_body .= elgg_view("input/dropdown", array("name" => "parent_guid", "options_values" => $parent_options, "value" => elgg_get_sticky_value("static", "parent_guid", $parent_guid))) . "</div>";
} else {
...
$form_body .= elgg_view("input/select", array("name" => "enable_comments", "value" => elgg_get_sticky_value("static", "enable_comments", $content_enable_comments), "options_values" => $comment_options)) . "</div>";

$form_body .= "<div><label>" . elgg_echo("static:new:moderators") . "</label><br />";
$form_body .= elgg_view("input/userpicker", array("name" => "moderators", "values" => elgg_get_sticky_value("static", "moderators", $content_moderators)));

$form_body .= "<div><label>" . elgg_echo("access") . "</label><br />";
$form_body .= elgg_view("input/access", array("name" => "access_id", "value" => elgg_get_sticky_value("static", "access_id", $content_access_id))) . "</div>";
